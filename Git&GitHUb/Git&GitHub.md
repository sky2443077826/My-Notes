...
- [Git&GitHub学习笔记](#gitgithub学习笔记)
  ...
  - [* 分支管理：多生产线同时推进](#-分支管理多生产线同时推进)
  - [二、GIT](#二git)
    - [2.1 Git本地结构](#21-git本地结构)
    - [2.2 Git和代码托管中心](#22-git和代码托管中心)
    - [2.3 本地库和远程库交互](#23-本地库和远程库交互)
  - [三、Git命令行操作](#三git命令行操作)
    - [3.1 本地库初始化](#31-本地库初始化)
    - [3.2 设置签名](#32-设置签名)
    - [3.3 常用操作](#33-常用操作)
      - [3.3.1 查看状态](#331-查看状态)
      - [3.3.2 添加到暂存区](#332-添加到暂存区)
      - [3.3.3 提交到工作区](#333-提交到工作区)
      - [3.3.4 查看历史状态](#334-查看历史状态)
      - [3.3.5 版本的前进后退](#335-版本的前进后退)
      - [3.3.6 reset命令的参数](#336-reset命令的参数)
      - [3.3.7 文件的比较](#337-文件的比较)
    - [3.4 分支管理](#34-分支管理)
      - [3.4.1  什么是分支？](#341--什么是分支)
      - [3.4.2  分支的好处？](#342--分支的好处)
      - [3.4.2  分支的操作](#342--分支的操作)
  - [四、Git原理](#四git原理)
    - [4.1 哈希算法](#41-哈希算法)
    - [4.2 Git分支管理机制](#42-git分支管理机制)
      - [4.1 分支的创建](#41-分支的创建)
      - [4.1 分支的切换](#41-分支的切换)
  - [五、GitHub](#五github)
    - [5.1 创建远程仓库](#51-创建远程仓库)
    - [5.2 一些基本操作](#52-一些基本操作)
...
## 二、GIT
### 2.1 Git本地结构
![](images/mmexport1632271907222.png)
*简单示意图*
* 工作区-写代码
  ```git
    //使用此命令，将代码添加到暂存区
...
* 在版本控制中，使多条线路并行推进多个任务。
  ![分支图](images/mmexport1632280418607.png)
  *简单分支流程图*
...
验证文件的原理
![](images/[8WZG787R8]ZJ~VN05~AMRK.png)

### 4.2 Git分支管理机制
#### 4.1 分支的创建
* 创建一个指针（master）指向版本
* 再创建一个指针（testing）也指向相同版本
![](images/IR5DC8DK{6`DD9FC0XCTA{O.png)
